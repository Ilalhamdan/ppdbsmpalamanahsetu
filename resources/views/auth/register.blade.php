...
<head>
    ...
    <title>Registrasi PPDB - SMP AL-AMANAH</title>
    ...
    <style>
        ...
        .form-label { font-size: 12px; margin-bottom: 4px; font-weight: 500; color: rgba(255, 255, 255, 0.9); }
    </style>
</head>
<body class="d-flex align-items-center justify-content-center" style="min-height: 100vh; padding: 20px 0;">
    ...
    <div class="card-register text-center">
        ...
        <p class="text-warning-50 mb-3 text-uppercase tracking-wider fw-semibold" style="font-size: 10px; color: rgba(254, 240, 138, 0.8);">Registrasi Calon Siswa</p>
        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <div class="mb-2.5 text-start">
                ...
                <input type="email" name="email" class="form-control form-control-kustom @error('email') is-invalid @enderror" value="{{ old('email') }}" required placeholder="Masukkan email aktif">
                @error('email') <span class="text-danger" style="font-size: 11px;">{{ $message }}</span> @enderror
            </div>

            <div class="mb-2.5 text-start">
                <label class="form-label">Username</label>
                <input type="text" name="username" class="form-control form-control-kustom @error('username') is-invalid @enderror" value="{{ old('username') }}" required placeholder="Masukkan username unik">
                @error('username') <span class="text-danger" style="font-size: 11px;">{{ $message }}</span> @enderror
            </div>


            <div class="mb-2.5 text-start">
                <label class="form-label">Password</label>
                <input type="password" name="password" class="form-control form-control-kustom @error('password') is-invalid @enderror" required placeholder="Minimal 8 karakter">
                @error('password') <span class="text-danger" style="font-size: 11px;">{{ $message }}</span> @enderror
            </div>

            <div class="mb-3.5 text-start">
                <label class="form-label">Konfirmasi Password</label>
                <input type="password" name="password_confirmation" class="form-control form-control-kustom" required placeholder="Ulangi password Anda">
            </div>

            <div class="d-grid mb-2.5">
                <button type="submit" class="btn btn-daftar">Daftar Akun Baru</button>
            </div>

            <div>
                <a href="{{ route('login') }}" class="text-link">Sudah memiliki akun? <strong>Login</strong></a>
            </div>
        </form>
    </div>
